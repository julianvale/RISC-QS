"""RFSoC4x2 oscilloscope test for one or two PMODA-1 laser events.
...
def laser_burst(out: Array, half_period: int, duration: int, cw: int,
    ...
    if second_gap_batches > 0:
        play_laser(half_period, duration, cw, start + duration + second_gap_batches)  # noqa: F821
        wait_until(start + 2 * duration + second_gap_batches + SETTLE_TICKS)  # noqa: F821
    else:
...
def _validate(half_period: int, duration: int, cw: int, second_gap_batches: int = 0) -> None:
    ...
    if second_gap_batches < 0:
        raise ValueError("second-gap-batches must be non-negative")


def run_once(host: str, port: int, half_period: int, duration: int, cw: int,
             timeout_s: float = 2.0, second_gap_batches: int = 0):
    """Load and run the core-0 laser test, returning its recorded schedule values."""
    _validate(half_period, duration, cw, second_gap_batches)
    if timeout_s <= 0:
    ...
    try:
        ...
        return run.rerun(
            ...
            params={0: {"half_period": half_period, "duration": duration, "cw": cw,
                        "second_gap_batches": second_gap_batches}},
...
def main(argv=None):
    ...
    parser.add_argument("--second-gap-batches", type=int, default=0,
                        help="emit one additional identical event after this gap; 0 emits one event")
    parser.add_argument("--timeout-s", type=float, default=2.0)
    args = parser.parse_args(argv)

    result = run_once(args.host, args.port, args.half_period, args.duration, int(args.cw), args.timeout_s,
                      args.second_gap_batches)
    ...
    events = 2 if args.second_gap_batches else 1
    print(f"scheduled {events} laser event(s): {mode}, duration={duration / dsp_hz * 1e6:.3f} us "
