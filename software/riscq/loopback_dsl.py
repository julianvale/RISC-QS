...
def run_loopback(profile=None, timeout_s: float = 2.0):
    drv = ProgramDriver.connect(profile)
    gate, demod = _tables(drv.map)
    program = compile_kernel(
        k_loopback, drv.map, tables={"gate": gate, "demod": demod}, out=Array(8),
    ...
    try:
        for mode, name in enumerate(MODE_NAMES):
            result = run.rerun(
                drv, drv.map, {0: program}, params={0: {"mode": mode}},
...
def main(argv=None):
    ...
    run_loopback(args.profile, args.timeout_s)
