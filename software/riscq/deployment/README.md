...
call; the active release context owns platform files, while `engine.py` owns aligned word loading,
firmware-manifest-declared envelope placement, complete image
...
or offsets. The board service uses the `current` release symlink, private-link bind address, pairing
