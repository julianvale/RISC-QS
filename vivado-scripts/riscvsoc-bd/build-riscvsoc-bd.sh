...
#
# Self-contained: it generates the BD (vivado=true) RTL with GenPulseTableSocVivado, then runs the BD
# assembly in flow-bd.tcl + inc/{config,create-project,package-ip,bd-build,bd-finalize,run}.tcl, with the
# floorplan from pblocks-bd.tcl. RTL, the Vivado project, the reports and vivado.log all land in one
# folder under the repo-root build/ — <repo>/build/<PROJ> — so several designs can build in parallel.
#
...
#
# Usage:
#   ./build-riscvsoc-bd.sh                    # 14q, full floorplan, synth+impl in the BD
#   RISCQ_QUBITS=3 ./build-riscvsoc-bd.sh # smaller config
...
#
# Env: RISCQ_VIVADO_BIN, RISCQ_QUBITS, RISCQ_SKIP_GEN, RISCQ_PROJ_NAME (default riscvsoc-bd),
#   plus the floorplan knobs read by pblocks-bd.tcl: RISCQ_{ROW,PERROW,CONFINE}, RISCQ_BD_BASE,
#   and RISCQ_PLACE_DIRECTIVE (default ExtraNetDelay_high — the placer directive). See README.md.
# ============================================================================================
set -e
...
# 1) RTL — the BD (vivado=true) form, keepCoreHierarchy=true — emitted INTO the project build dir.
if [ "${RISCQ_SKIP_GEN:-0}" != "1" ]; then
  echo "[riscvsoc-bd] generating BD RTL (GenPulseTableSocVivado qubitNum=$QUBITS, vivado=true, keepCoreHierarchy=true) → $BUILD"
  "$REPO_DIR/.metals/mill" runMain riscq.soc.GenPulseTableSocVivado "$QUBITS" "$BUILD"
else
...
echo "[riscvsoc-bd] building block design in $BUILD (floorplan, IP retiming, place=$RISCQ_PLACE_DIRECTIVE / route=AggressiveExplore) …"
