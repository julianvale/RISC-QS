...
# Run stages. ZCU216 preserves its legacy full-build defaults. RFSoC4x2 creates and validates the
# project/block design by default; enable normal stages explicitly with RISCQ_RUN_*.
set _legacy_full_build [expr {$PLATFORM eq "zcu216"}]
set GENERATE_TARGETS $_legacy_full_build
set RUN_SYNTH     $_legacy_full_build
set RUN_IMPL      $_legacy_full_build
set RUN_BITSTREAM $_legacy_full_build
...
if {$RUN_IMPL}      { set RUN_SYNTH 1 }
if {$RUN_SYNTH}     { set GENERATE_TARGETS 1 }
...
puts "\[config\] dsp=${DSP_FREQ}Hz host=${HOST_FREQ}Hz targets=$GENERATE_TARGETS synth=$RUN_SYNTH impl=$RUN_IMPL bit=$RUN_BITSTREAM"
